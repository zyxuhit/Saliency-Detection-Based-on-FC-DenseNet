...
def create_model(name, batch_size, learning_rate = 0.0001, wd = 0.00001, concat = False, l2_loss = False, penalty = False, coef = 0.05, verbosity = 0):
  """
...
def do_train(model, sess, stream_input, stream_input_aux, max_step, log_folder, mode, weight_file = None, model_to_copy = None, model_copy_is_concat = False, valid = True, dataset = None, save_copy = False):
  ...
  if valid:
    ...
    for thres in threshold:
      predicted_class = tf.where(guess_aux*255 > thres, ones, zeros)
      ...
      liste.append(tf.stack([precision,recall]))
    result = tf.stack(liste)   
    adaptive_threshold = (2*tf.reduce_mean(guess_aux,[0,1],keep_dims= True))
    adaptive_output = tf.where(guess_aux > adaptive_threshold, ones, zeros)
...
def compute_score(model, sess, stream_input, restore_path, dataset, split, write = False, save = False):
  ...
  liste = []
  for t in threshold:
    predicted_class = tf.where(guess*255 > t, ones, zeros)
  ...
  adaptive_output    = tf.where(guess > adaptive_threshold, ones, zeros)
  ...
  if save: #Save result images
    path = restore_path + '/' + model.name+'/'+str(ret_bis) 
  ...
  for step in range(num_iter): #Compute score
    ...
    result_ret, adaptive_output_ret,adaptive_precision_ret, adaptive_recall_ret, adaptive_f_measure_ret, names_ret, images_ret, labels_ret, guess_ret = sess.run([result, adaptive_output,adaptive_precision, adaptive_recall, adaptive_f_measure, names, images, labels, guess])
    
    for i in range(stream_input.batch_size):
      
      if save: #Save result images
        ret_path = path + names_ret[i].split('.')[0]
        ...
        Image.fromarray(ret, 'P').save(ret_path + '_' + 'Ours' + '.png')
        ret = np.asarray( adaptive_output_ret[i]*255, dtype="int8" )
        Image.fromarray(ret, 'P').save(ret_path + '_' + '01' + '.png')
  ...
  if write: #Save score
    file = open(restore_path + '/' + model.name + "/" +str(ret_bis)+'/'+ dataset + ".txt" , 'w')
    file.write('model name is ' + model.name + '\n')
...
def compute_inter(model, sess, stream_input, restore_path, dataset, split, arithmetic):
  ...
  for step in range(num_iter):
    ...
    for i in range(model.batch_size):
      ...
      ret_path = path + str(ret_bis) + '_' + str(name_ret[i])
      ret = np.asarray( images_ret[i]*255, dtype="int8" )
